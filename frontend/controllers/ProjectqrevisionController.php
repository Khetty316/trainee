...
//                $mpdf = $this->generatePdf($model, $revision);
    ...
    public function actionDeleteQuotationPdf($id) {
        ...
        if (!$model) {
            FlashHandler::err("Quotation not found.");
        ...
        $quotationEmails = QuotationEmails::find()->where(['quotation_id' => $model->id])->all();

        foreach ($quotationEmails as $email) {

            // Get all attachments for this email
            $attachments = QuotationEmailAttachments::findAll(['email_id' => $email->id]);

            foreach ($attachments as $attachment) {

                // Full path to the file
                $filePath = Yii::getAlias(Yii::$app->params['quotation_pdf_path'] . $attachment->file_name);
        ...
        // Finally delete quotation PDF master
        if ($model->delete()) {
            FlashHandler::success("Quotation removed.");
        } else {
            FlashHandler::err("Failed to remove. Kindly call for support.");
        }
    ...
    public function actionUpdateQuotationEmail($id, $email_type) {
        ...
        if ($email_model->load(Yii::$app->request->post())) {
            if ($email_model->validate()) {
                ...
                // Validate sender
                if (!$email_model->validateEmailAddress($postEmail['sender'])) {
                    $validationErrors[] = 'Invalid sender email address';
                }

                // Validate recipient
                if (!$email_model->validateEmailAddress($postEmail['recipient'])) {
                    $validationErrors[] = 'Invalid recipient email address';
                }

                // Validate Cc emails
                if (!empty(trim($postEmail['Cc']))) {
                    $ccEmails = array_map('trim', explode(',', $postEmail['Cc']));
                    foreach ($ccEmails as $ccEmail) {
                        if (!empty($ccEmail) && !$email_model->validateEmailAddress($ccEmail)) {
                            $validationErrors[] = "Invalid Cc email address: $ccEmail";
                        }
                    }
                }

                // Validate Bcc emails
                if (!empty(trim($postEmail['Bcc']))) {
                    $bccEmails = array_map('trim', explode(',', $postEmail['Bcc']));
                    foreach ($bccEmails as $bccEmail) {
                        if (!empty($bccEmail) && !$email_model->validateEmailAddress($bccEmail)) {
                            $validationErrors[] = "Invalid Bcc email address: $bccEmail";
                        }
                    }
                }
                ...
                // Automatically add sender to Cc list
                $ccEmails = [];

                // Add existing Cc emails if any
                if (!empty(trim($postEmail['Cc']))) {
                    $ccEmails = array_map('trim', explode(',', $postEmail['Cc']));
                } elseif (!empty($oldCc)) {
                    $ccEmails = array_map('trim', explode(',', $oldCc));
                }

                // Add sender email to Cc if not already present
                if (!in_array($email_model->sender, $ccEmails)) {
                    $ccEmails[] = $email_model->sender;
                }

                // Save Cc list back to model (remove empty values)
                $email_model->Cc = implode(', ', array_filter($ccEmails));
                ...
                if (!$email_model->save(false)) {
                    FlashHandler::err($email_model->getErrors());
                    return $this->render('_email_form', [
                                'id' => $id,
                                'email_type' => $email_model->email_type,
                                'client_id' => $email_model->client_id,
                ...
                try {
                    // Add system-generated footer to email content
                    $systemFooter = "\n\nThis email was automatically generated by system.\n";
                    $systemFooter .= "Sent on: " . date('d/m/Y H:i:s') . "\n"; // DD/MM/YYYY format

                    $emailContent = $email_model->content . $systemFooter;
                    ...
                    // Only set Cc if not empty
                    if (!empty($CcList)) {
                        $mail->setCc($CcList);
                    }

                    // Only set Bcc if not empty
                    if (!empty($BccList)) {
                        $mail->setBcc($BccList);
                    }
                    ...
                    if ($uploadedFiles) {
                        foreach ($uploadedFiles as $file) {
                            ...
                            if (!$attachment->save()) {
                                FlashHandler::err($attachment->getErrors());
                            }
                    ...
                    if ($mail->send()) {
//                        FlashHandler::success('Email processing completed. Please check your inbox to confirm delivery.');
                        \Yii::$app->session->setFlash('info', 'Email processing completed. Please check your inbox to confirm delivery.');
                        ...
                        FlashHandler::err('Failed to process email. Please try again.');
                    }
                } catch (\Exception $e) {
                    Yii::error('Email sending failed: ' . $e->getMessage(), __METHOD__);
                    FlashHandler::err('Failed to send email: ' . $e->getMessage());
                }
            } else {
                Yii::error($email_model->getErrors(), __METHOD__);
                FlashHandler::err('Invalid email data');
            }
        }
    ...
    public function actionUserManualSendingEmail() {
        $this->layout = false;
        $fileName = QuotationEmails::USER_MANUAL_SENDING_EMAIL_FILENAME;
    ...
    public function actionCheckEmailExists() {
        ...
        if (!checkdnsrr($domain, "MX") && !checkdnsrr($domain, "A")) {
            return [
                'success' => false,
                'error' => [
                    'type' => 'Domain does not exist or cannot receive emails'
                ]
            ];
        }
