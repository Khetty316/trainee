...
$this->params['breadcrumbs'][] = ['label' => 'Claim Review - Finance', 'url' => ['finance-approval-pending']];
                    ...
                    <table class="table table-bordered mb-0">
                        <thead class="table-dark">
                            <tr>
                                ...
                                <th width="20%">Finance's Response</th>
                                <th width="10%">Superior's Response</th>
                                <th width="10%">Payment</th>
                            </tr>
                        </thead>
                        <tbody>
...
<script>
    $(document).ready(function () {
        $('.decision-wrapper').each(function () {
            ...
            const paymentProof = $(`[name="ClaimApprovalWorklist[${index}][scannedFile]"]`);
            ...
            radios.on('change', function () {
                ...
                const currentClaimStatus = '<?= $model->claim_status ?>'; // Convert PHP to JS
                ...
                // Show payment proof only if claim is waiting for payment AND user selects approved
                if (currentClaimStatus === '<?= frontend\models\RefGeneralStatus::STATUS_WaitingForPayment ?>' && value === '<?= ClaimMaster::STATUS_APPROVED ?>') {
                    paymentProof.show().attr('required', true);
                } else {
                    paymentProof.hide().val('').removeAttr('required');
                }
