...
/**
 * This is the model class for table "petty_cash_request_pre".
 *
 * @property int $id
 * @property int|null $pc_request_master_id
 * @property float|null $amount_requested
 * @property string|null $purpose_of_advance
 ...
 */
class PettyCashRequestPre extends \yii\db\ActiveRecord {

    const USER_MANUAL_FILENAME = "T6B4-Petty Cash Module-00.pdf";
    
    /**
     * {@inheritdoc}
     */
    public static function tableName() {
        return 'petty_cash_request_pre';
    }
    ...
    public function rules() {
        return [
            [['pc_request_master_id', 'status', 'created_by', 'updated_by', 'deleted_by', 'responsed_by'], 'integer'],
            [['amount_requested', 'amount_approved'], 'number'],
            [['created_at', 'updated_at', 'deleted_at', 'responsed_at'], 'safe'],
            [['purpose_of_advance', 'responsed_remark'], 'string', 'max' => 255],
    ...
    public function attributeLabels() {
        return [
            ...
            'amount_requested' => 'Amount Requested (RM)',
            'purpose_of_advance' => 'Purpose Of Advance',
            'amount_approved' => 'Amount Approved (RM)',
            'status' => 'Status',
    ...
    public function getPettyCashRequestPosts() {
        return $this->hasMany(PettyCashRequestPost::className(), ['pc_request_pre_id' => 'id']);
    }
