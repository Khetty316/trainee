...
//                                $panel->checkAndGetElecTask();
                                    $progress = $panel->getElecTaskProgressStatus(); // Pull record from task assigned
                                    $taskAssignElecs = $panel->checkTaskAssignElec();
                                    $isFinalized = !empty($panel->finalized_at . $panel->design_completed_at);
                                    $taskWeightIsValid = $panel->checkTotalElecTaskWeight();
                                    ?>
                                    <tr>
                                        <td>
                                            <?php
...
//                                        echo $panel->project_production_panel_code;
                                        ...
                                        <td class="text-right tdnowrap"> 
                                            <?= $panel->elecWorkStatus->status_name ?? null ?>
                                        </td>
                                        <td class="text-right tdnowrap"> 
                                            <?= MyFormatter::asDecimal2_emptyZero($panel->elec_complete_percent) ?> %
                                        </td>
                                        <td class="text-right tdnowrap"> 
                                            <?= MyFormatter::asDecimal2_emptyZero($panel->elec_assign_percent) ?> %
                                        </td>
                                        <?php
                                        if (!empty($progress)) {
                                            foreach ((array) $toDos as $key => $toDo) {
                                                ...
                                                if (!empty($progress[$code])) {
                                                    foreach ($taskAssignElecs as $key => $taskAssignElec) {
                                                        if ($taskAssignElec->prod_elec_task_id === $progress[$code]['id']) {
                                                            $hideDeleteIcon = true;
                                                        }
                                                    }

                                                    if (!$progress[$code]['allAssigned'] && Yii::$app->user->can(AuthItem::ROLE_PrdnElec_Executive) && $taskWeightIsValid) {
                                                    ...
                                                    if (!$taskWeightIsValid) {
                                                        if ($progress[$code]['hasRecord'] || $hideDeleteIcon) {
                                                            ...
                                                            echo '<i class="fas fa-list ml-2 text-secondary"></i>';
                                                            echo Html::a('<i class="fas fa-trash ml-2" style="color: red;"></i>', ['delete-elec-task', 'taskId' => $progress[$code]['id'], 'id' => $model->id], [
