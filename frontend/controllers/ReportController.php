...
//    }
    ...
    $isInternalValue = ($isInternal === '' ? null : intval($isInternal));

    $fabOverall = $this->getTopPerformanceFactoryStaffOverall($dateFrom, $dateTo, $isInternalValue, TaskAssignment::taskTypeFabrication);
    $fabByTask = $this->getTopPerformanceFactoryStaffByTaskCode($dateFrom, $dateTo, $isInternalValue, TaskAssignment::taskTypeFabrication);

    $elecOverall = $this->getTopPerformanceFactoryStaffOverall($dateFrom, $dateTo, $isInternalValue, TaskAssignment::taskTypeElectrical);
    $elecByTask = $this->getTopPerformanceFactoryStaffByTaskCode($dateFrom, $dateTo, $isInternalValue, TaskAssignment::taskTypeElectrical);
    ...
    $this->storeInDatabase($cacheKey, $reportData, $periodKey, $dateFrom, $dateTo, $isInternalValue);
}
    ...
    private function getTopPerformanceFactoryStaffOverall($reportDetailByTask) {
        if (empty($reportDetailByTask)) {
            return [];
        }

        $overall = [];

        foreach ($reportDetailByTask as $taskCode => $staffList) {
            foreach ($staffList as $staff) {
                ...
                // Add this task's totalPerformance to the staff total
                $overall[$id]['totalPerformance'] += floatval($staff['totalPerformance']);
            }
        }
        ...
        $totalSum = array_sum(array_column($overall, 'totalPerformance'));

        $top = array_slice($overall, 0, 3);

        foreach ($top as $index => &$staff) {
            $staff['rank'] = $index + 1;
            $staff['percentage'] = ($totalSum > 0) ? round(($staff['totalPerformance'] / $totalSum) * 100, 4) : 0;
            $staff['formattedValue'] = number_format($staff['totalPerformance'], 4);
        }
...
//        $reportDetail = $model->calculateAllDepartmentCompleteRecord($dateFrom, $dateTo, $isInternal, $department);
//
...
//                        'ROUND(SUM(amount / totalStaff * qty_completed_panel_individual / qty_total_panel * panel_type_weight / 100 * single_task_weight / 100), 4) AS totalPerformance'
//                    ])
...
//        }
//
//        $query->where(['between', 'complete_date', $dateFrom, $dateTo])
...
//                    ->where(['worker_task_categories.task_type' => TaskAssignment::taskTypeFabrication])
...
//                    ->where(['worker_task_categories.task_type' => TaskAssignment::taskTypeElectrical])
//                    ->groupBy(['worker_task_categories.user_id']);
//        }
//
...
//                        'MAX(user.staff_id) as staffId',
//                        'MAX(user.fullname) as fullname',
//                        'worker_task_categories.task_code as fab_task_code',
//                        'ROUND(COALESCE(SUM(amount / totalStaff * qty_completed_panel_individual / qty_total_panel * panel_type_weight / 100 * single_task_weight / 100), 0), 4) AS totalPerformance'
//                    ])
//                    ->from('worker_task_categories')
//                    ->leftJoin('user', 'user.id = worker_task_categories.user_id')
...
//                        'MAX(user.staff_id) as staffId',
//                        'MAX(user.fullname) as fullname',
//                        'worker_task_categories.task_code as elec_task_code',
//                        'ROUND(COALESCE(SUM(amount / totalStaff * qty_completed_panel_individual / qty_total_panel * panel_type_weight / 100 * single_task_weight / 100), 0), 4) AS totalPerformance'
//                    ])
//                    ->from('worker_task_categories')
//                    ->leftJoin('user', 'user.id = worker_task_categories.user_id')
