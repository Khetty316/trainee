...
/* @var $searchModel frontend\models\ProjectProduction\fabrication\TaskAssignFabSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

//$this->title = 'Task Assign Fabs';
//$this->params['breadcrumbs'][] = $this->title;
//        $task = \frontend\models\ProjectProduction\fabrication\ProductionFabTasks::findOne($taskId);

$project = $task->projProdPanel->projProdMaster;

$this->title = $task->projProdPanel->project_production_panel_code . " - " . ($task->fabTaskCode->name);
                    ...
                    'columns' => [
                        ...
                        [
                            ...
                            'value' => function ($model) use ($task) {
                                $details = \frontend\models\projectproduction\fabrication\TaskAssignFabTargetDateTrial::find()
                                        ->where(['task_assign_fab_id' => $model->id])
...
//                                    if ((Yii::$app->user->can(AuthItem::ROLE_Director) || $model->created_by == Yii::$app->user->id) && $model->active_sts) {
                                    if ((MyCommonFunction::checkRoles([AuthItem::ROLE_PrdnFab_Executive, AuthItem::ROLE_Director]) || $model->created_by == Yii::$app->user->id) && $model->active_sts) {
...
//                            }
                        ...
                        [
                            ...
                            'format' => 'raw',
                            'value' => function ($model) {
                                $staffList = $model->taskAssignFabStaff;
                                $returnStr = "";
                        ...
                        [
                            ...
                            'value' => function ($model) {
                                ...
                                $staffMembers = $model->taskAssignFabStaff;
                                ...
                                foreach ($staffMembers as $staff) {
                                    if ($staff->taskAssignFabStaffCompletes) {
                                        $allCompleteTasks = array_merge($allCompleteTasks, $staff->taskAssignFabStaffCompletes);
                                    }
                                }

                                // Get deleted tasks
                                foreach ($staffMembers as $staff) {
                                    $deleted = frontend\models\projectproduction\fabrication\TaskAssignFabStaffCompleteDelete::find()
                                            ->where(['task_assign_fab_staff_id' => $staff->id])
                                            ->all();
                                ...
                                if (!empty($mergedArray)) {
                                    foreach ($mergedArray as $item) {
                                        $taskAssignFabStaff = frontend\models\ProjectProduction\fabrication\TaskAssignFabStaff::findOne($item->task_assign_fab_staff_id);
                                        $assignedStaff = User::findOne($taskAssignFabStaff->user_id);
                                        if (isset($item->deleted_at)) {
                                            ...
                                            if ($model->active_sts && MyCommonFunction::checkRoles([AuthItem::ROLE_PrdnFab_Executive, AuthItem::ROLE_Director])) {
                                                $text .= '<td>' . Html::a('Revert <i class="fas fa-undo"></i>', 'javascript:void(0)', [
                                                            'value' => 'ajax-action-revert-set-complete?id=' . $item->id . '&taskId=' . $item->task_assign_fab_staff_id,
...
//                                $text = '';
//                                $completeTasks = $model->taskAssignFabCompletes;
//                                $completeTaskDeleted = $model->taskAssignFabCompleteDeletes;
...
//                                            if ($model->active_sts && MyCommonFunction::checkRoles([AuthItem::ROLE_PrdnFab_Executive, AuthItem::ROLE_Director])) {
//                                                $text .= '<td>' . Html::a('Revert <i class="fas fa-undo"></i>', 'javascript:void(0)', [
//                                                            'value' => 'ajax-action-revert-set-complete?id=' . $item->id . '&taskId=' . $item->task_assign_fab_id,
...
//                        'value' => function ($model) {
//                            $text = '';
//                            $completeTaskTrails = $model->taskAssignFabCompleteTrails;
//                            $completeTaskDeleted = $model->taskAssignFabCompleteDeletes;
//
...
//                                            if ($deletedTask->task_assign_fab_complete_id === $completeTaskTrail->task_assign_fab_complete_id) {
//                                                $isDeleted = true;
...
//                                        if ($model->active_sts && MyCommonFunction::checkRoles([AuthItem::ROLE_PrdnFab_Executive, AuthItem::ROLE_Director])) {
//                                            $text .= '<td>' . Html::a('Revert <i class="fas fa-undo"></i>', 'javascript:void(0)', [
//                                                        'value' => 'ajax-action-revert-set-complete?id=' . $completeTaskTrail->task_assign_fab_complete_id . '&taskId=' . $completeTaskTrail->task_assign_fab_id,
//                                                        'class' => 'btn btn-sm btn-danger modalButtonMedium',
//                                                        'title' => "Revert Completed Panel Amount",
//                                                        'data-modaltitle' => "Revert Completed Panel Amount"])
//                                                    . '</td>';
...
//                            if ((Yii::$app->user->can(AuthItem::ROLE_Director) || $model->created_by == Yii::$app->user->id)) {
                            if ((MyCommonFunction::checkRoles([AuthItem::ROLE_PrdnFab_Executive, AuthItem::ROLE_Director]) || $model->created_by == Yii::$app->user->id)) {
                                return Html::a("<i class='far fa-edit text-success'></i>", 'javascript:void(0)', ['value' => 'ajax-make-complaint?complaintId=' . $model->id, 'class' => 'modalButton mb-2'])
