    ...
    <?php if (!empty($pendingDispatch)) { ?>
        <fieldset class="border p-1">
            ...
            ActiveForm::begin([
                'id' => 'myForm',
                'method' => 'post',
                'action' => yii\helpers\Url::to(['dispatch-acknowledgement', 'productionPanelId' => $dispatchMaster->production_panel_id, 'dispatchId' => $dispatchMaster->dispatch_id]),
            ]);
                            ...
                            <table class="table table-sm table-bordered mb-0">
                                ...
                                <tbody>
                                    <?php foreach ($pendingDispatch as $key => $dispatch): ?>
                                        <?php
                                        $detail = StockOutboundDetails::findOne($dispatch['stock_outbound_details_id']);
                                        ...
                                        <tr style="<?= ($detail->active_sts == 0) ? 'text-decoration: line-through; color: red;' : '' ?>">
                                            ...
                                            <?php if ($status !== null && $detail->active_sts == 1) { ?>
                                                <?= Html::hiddenInput("dispatch[{$dispatch['trial_id']}][detailId]", $dispatch['stock_outbound_details_id']) ?>
                                            <?php } ?>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
    ...
    <?php } else if (!empty($pendingAdjust) || !empty($pendingReturn)) { ?>
        <fieldset class="border p-1">
            ...
            ActiveForm::begin([
                'id' => 'myForm',
                'method' => 'post',
                'action' => yii\helpers\Url::to(['adjust-and-return-acknowledgement', 'productionPanelId' => $dispatchMaster->production_panel_id, 'dispatchId' => $dispatchMaster->dispatch_id]),
            ]);
                                ...
                                <tbody>
                                    ...
                                    if (!empty($pendingAdjust)) {
                                        foreach ($pendingAdjust as $key => $adjust):
                                            $detail = StockOutboundDetails::findOne($adjust['stock_outbound_details_id']);
                                            ...
                                            <tr style="<?= ($detail->active_sts == 0) ? 'text-decoration: line-through; color: red;' : '' ?>">
                                                ...
                                                <td><?= $detail->brand ?></td>
                                                <td><?= $detail->descriptions ?></td>
                                                <td><?= $adjust['dispatch_qty'] ?></td>
                                                <td><?= $adjust['remark'] ?></td>
                                                <td><?= $createdBy->fullname ?> @ <?= MyFormatter::asDateTime_ReaddmYHi($adjust['trial_created_at']) ?></td>
                                                <td><span class="text-warning"><?= ($adjust['current_sts'] == StockDispatchMaster::TO_BE_COLLECTED ? 'To Be Collected' : 'To Be Acknowledged') ?></span></td>
                                                <td><?= MyFormatter::asDateTime_ReaddmYHi($adjust['status_updated_at']) ?></td> 
                                                <?php if ($status !== null && $detail->active_sts == 1) { ?>
                                                    <td class="text-center">
                                                        <?php if ($adjust['current_sts'] == StockDispatchMaster::TO_BE_ACKNOWLEDGED) { ?>
                                                            <input type="checkbox" 
                                                                   class="select-row" 
                                                                   data-master="adjust" 
                                                                   data-id="<?= $adjust['trial_id'] ?>">
                                                               <?php } ?>
                                                    </td>
                                                    <?=
                                                    Html::hiddenInput("adjust[{$adjust['trial_id']}][detailId]", $adjust['stock_outbound_details_id'], [
                                ...
                                <tbody>
                                    ...
                                    if (!empty($pendingReturn)) {
                                        foreach ($pendingReturn as $key => $return):
                                            $detail = StockOutboundDetails::findOne($return['stock_outbound_details_id']);
                                            ...
                                            <tr style="<?= ($detail->active_sts == 0) ? 'text-decoration: line-through; color: red;' : '' ?>">
                                                ...
                                                <?php if ($status !== null && $detail->active_sts == 1) { ?>
                                                    ...
                                                    <?=
                                                    Html::hiddenInput("return[{$return['trial_id']}][detailId]", $return['stock_outbound_details_id'], [
