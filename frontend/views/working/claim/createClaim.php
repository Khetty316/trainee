...
$this->title = 'Create Claims Detail';
...
//                'action' => '/project/newquotation',
    ...
    ?>
    <?= $form->field($model, 'claimant_id')->hiddenInput(["value" => Yii::$app->user->id])->label(false) ?>
    ...
    <div class="form-row">
        <div class="col-sm-12 col-md-4">
            <?= $form->field($model, 'scannedFile')->fileInput()->label('Reference.<br/><span class="text-primary" style="font-size:10pt">Recommended to upload image/pdf as reference for prior processing</span>') ?>
        </div>
    </div>
    ...
    <div class="form-row">
        ...
        <div class="col-sm-12 col-md-2 forTravel">
            <?=
                    ...
                    ->textInput(['type' => 'number', 'style' => 'text-align:right', 'step' => ".01", 'value' => '50.00'])
                    ->label("Amount/Day (RM)")
            ?>
        </div>
    </div>
    ...
    <div class="form-row">
        ...
        <div class="col-sm-12 col-md-2">
            <?= $form->field($model, 'amount')->textInput(['type' => 'number', 'style' => 'text-align:right', 'step' => ".01"])->label('Amount (RM)') ?>
        </div>
    </div>
    ...
    <div class="form-row" id="totalAmountDiv" style="display:none">
    ...
    <div class="form-row">
        <div class="col-sm-12 col-md-2">
            ...
            if (!$superior) {
                ...
                echo $form->field($model, "tempAuthorizeName")->textInput(['value' => $superior->fullname,'disabled'=>true])->label("Authorized By");
                echo $form->field($model, 'authorized_by', ['options' => ['class' => 'hidden']])->hiddenInput(['value' => $superior->id]);
            }
            ?>
    ...
    <div class="form-row">
        <div class="col-sm-12 col-md-4 notForTravel">
    ...
    <div class="form-row">
        <div class="col-sm-12 col-md-8">
            ...
            <?= Html::button('Save & Next', ['class' => 'btn btn-success', 'onclick' => 'save("yes")']) ?>
        </div>
    </div>
    <?= $form->field($model, 'saveAndNext')->textInput(['class' => 'hidden'])->label(false) ?>
...
<script>
    ...
    function checkDate() {
        ...
        if (date1 === '' || date2 === '') {
            return 0;
        } else if (getDateFromRead(date1) > getDateFromRead(date2)) {
