...
//                        $totalAmtWithoutSST = ($totalAmount + $discountTotal);
                        ...
                        $totalSST = ($totalAmount) * $sst / 100;
                        $totalAmtWithSST = ($totalAmount) + $totalSST;
                        $totalAmtWithoutSST = ($totalAmount);

                        if ($model->with_sst) {
                            ?>
                            <td colspan="6" class='text-right bold italic' style='padding-right:10px'>
                                Sub-Total (<?= $model['currency']['currency_sign'] ?>) :  <br/>
                                <?php //= $model['discount_amt'] > 0 ? $discountDisplay : ""  ?>
                            ...
                            <td class='bold italic p5px text-right br bl'>
                                ...
                                <?php //= ($model['discount_amt'] > 0) ? MyFormatter::asDecimal2($discountTotal) . "<br/>" : "" ?>
