...
$getStatus = StockDispatchTrial::ADJUST_STATUS;
$dispatchedQty = ($dispatchMaster->unacknowledged_qty + $dispatchMaster->dispatched_qty);
$balance = $dispatchMaster->detail_qty - $dispatchedQty;
$limit = ($balance == 0 ? $dispatchMaster->total_trial_dispatch_qty : ($balance + $dispatchMaster->total_trial_dispatch_qty));
?>
...
<div class="stock-detail">
    ...
    $form = ActiveForm::begin([
                'id' => 'myForm',
                'method' => 'post',
                'action' => yii\helpers\Url::to(['adjust-dispatch-quantity', 'dispatchId' => $dispatchMaster->dispatch_id, 'detailId' => $dispatchMaster->stock_outbound_details_id]),
    ]);
    ?>
            ...
            <fieldset class="border p-1">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-lg-3 col-md-12 col-sm-12">
                        <?= $form->field($dispatchMaster, 'total_trial_dispatch_qty')->textInput(['disabled' => true])->label("Dispatch Quantity") ?>
                    </div>
                    <div class="col-lg-4 col-md-12 col-sm-12 mb-3">
                        <label for="addComplete-counter">Adjust Dispatch Quantity</label>
                        ...
                        <div id="error-message" class="invalid-feedback" style="display: none;">
                            Invalid input. The new dispatch quantity must be different from the initial dispatched quantity of <?= $dispatchMaster->total_trial_dispatch_qty ?>.
                        </div>
                    </div>
...
<script>
    $(document).ready(function () {
        const initialDispatchedQty = <?= $dispatchMaster->total_trial_dispatch_qty ?>;
        $('#counter-input').on('change', function (event) {
            const input = event.target;
            let errorMessage = document.getElementById('error-message');

            if (parseInt(input.value, 10) === initialDispatchedQty) {
                input.classList.add('is-invalid');
                errorMessage.textContent = 'Invalid input. The new dispatch quantity must be different from the initial dispatch quantity of <?= $dispatchMaster->total_trial_dispatch_qty ?>.';
                errorMessage.style.display = 'block';
            } else if (parseInt(input.value, 10) === 0 || !input.value.trim()) {
                input.classList.add('is-invalid');
