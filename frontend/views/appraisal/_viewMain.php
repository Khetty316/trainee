...
.table-scroll thead tr:nth-child(2) th.sticky-action{
    right: 0;
    z-index: 30;
    background: #fff !important;
}
...
<div class="table-scroll">
    <table class="table table-bordered table-striped table-hover m-0 mt-2 rounded">
        <thead>
    ...
    <!--            <th class="p-1">
        ...
        <tbody>
            <tr v-for="model in filteredModels" :key="model.id">
                ...
                <?php if ($super) { ?>
                    <td class="p-1 text-center sticky-action">
                        <a :href="'/appraisalgnrl/index-review?id=' + model.id"
                           class="btn btn-sm btn-info"
                           title="Click to view">View <i class="fas fa-eye"></i> </a>
                    </td>
                <?php } else { ?>
                    <td class="p-1 text-center sticky-action">
                        <a :href="'/appraisal/index-master?id=' + model.id"
                           class="btn btn-sm btn-info"
                           title="Click to view">View <i class="fas fa-eye"></i> </a>
                    </td>
                <?php } ?>
