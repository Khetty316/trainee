...
$this->title = ($module === 'finance' ? 'Petty Cash Replenishment - Finance' : 'Petty Cash Replenishment Approval - Director');
...
<div class="petty-cash-request-master-index">
    ...
    <p class="mt-2">
        <?php if ($module === 'finance') { ?>
            <?=
            Html::a("Request Replenishment",
                    "javascript:",
                    [
                        "onclick" => "event.preventDefault();",
                        "value" => \yii\helpers\Url::to(['request-replenishment']),
                        "class" => "modalButtonMedium btn btn-success",
                        'data-modaltitle' => "Petty Cash Replenishment Request"
                    ]
            )
            ?>
        <?php } ?>
...
//            'id',
            ...
            [
                ...
                'value' => function ($model) {
                    return \common\models\myTools\MyFormatter::asDecimal2($model->amount_requested);
                }
            ],
            [
            ...
            [
                ...
                'contentOptions' => ['class' => 'text-right'],
                'value' => function ($model) {
                    return \common\models\myTools\MyFormatter::asDecimal2($model->amount_approved);
                }
            ],
            [
                'attribute' => 'status',
                ...
                'filter' => false,
                'value' => function ($model) {
                    if (in_array($model->status, \frontend\models\RefGeneralStatus::STATUS_Pending_Finance) && $model->deleted_by === null) {
                        return '<span style="color:red;">' . $model->status0->status_name . '</span>';
                    }
            ...
            [
                ...
                'value' => function ($model) use ($module) {
                    if ($module === 'finance') {
                        $action = 'finance-view-replenishment-request';
                    } else {
                        $action = 'director-view-replenishment-request';
                    }
