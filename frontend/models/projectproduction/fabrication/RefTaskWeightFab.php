...
/**
 * This is the model class for table "ref_task_weight_fab".
 *
 * @property int $id
 ...
 *
 * @property RefProjProdTaskFab $taskCode
 * @property RefProjectQTypes $panelType
 */
class RefTaskWeightFab extends \yii\db\ActiveRecord {

    /**
     * {@inheritdoc}
     */
    public static function tableName() {
        return 'ref_task_weight_fab';
    }
    ...
    public function rules() {
        return [
            ...
            [['task_code'], 'exist', 'skipOnError' => true, 'targetClass' => RefProjProdTaskFab::className(), 'targetAttribute' => ['task_code' => 'code']],
    ...
    public function getTaskCode() {
        return $this->hasOne(RefProjProdTaskFab::className(), ['code' => 'task_code']);
    }
    ...
    public function updateRefTaskWeight($data, $paneltype) {
        foreach ($data as $taskCode => $value) {
            $model = RefTaskWeightFab::find()->where(['task_code' => $taskCode, 'panel_type' => $paneltype])->one();
