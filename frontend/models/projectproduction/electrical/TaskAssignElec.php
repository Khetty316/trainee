...
/**
 * This is the model class for table "task_assign_elec".
 *
 * @property int $id
 * @property int|null $proj_prod_panel_id
 * @property int|null $prod_elec_task_id
 ...
 */
class TaskAssignElec extends \yii\db\ActiveRecord {
    ...
    public function rules() {
        return [
            [['proj_prod_panel_id', 'prod_elec_task_id', 'quantity', 'complete_qty', 'start_date', 'complete_date', 'comments', 'created_at', 'created_by', 'updated_at', 'updated_by', 'deactivated_by', 'deactivated_at', 'complete_by', 'complete_at'], 'default', 'value' => null],
            [['active_sts'], 'default', 'value' => 1],
            [['proj_prod_panel_id', 'prod_elec_task_id', 'created_by', 'updated_by', 'active_sts', 'deactivated_by', 'complete_by'], 'integer'],
            ...
            [['prod_elec_task_id'], 'exist', 'skipOnError' => true, 'targetClass' => ProductionElecTasks::class, 'targetAttribute' => ['prod_elec_task_id' => 'id']],
...
//            [['remark_update_target_date', 'new_target_date'], 'required'],
    ...
    public function getTaskAssignElecCompletes() {
        return $this->hasMany(TaskAssignElecComplete::className(), ['task_assign_elec_id' => 'id']);
    }

    /**
     * Gets query for [[TaskAssignElecCompleteDeletes]].
     *
     * @return \yii\db\ActiveQuery
     */
    public function getTaskAssignElecCompleteDeletes() {
        return $this->hasMany(TaskAssignElecCompleteDelete::className(), ['task_assign_elec_id' => 'id']);
    }
    ...
    public function getTaskAssignElecStaff() {
        return $this->hasMany(TaskAssignElecStaff::className(), ['task_assign_elec_id' => 'id']);
    }
    ...
    public function getUsers() {
        return $this->hasMany(User::className(), ['id' => 'user_id'])->viaTable('task_assign_elec_staff', ['task_assign_elec_id' => 'id']);
    }
...
//            return false;
//        }
//        TaskAssignElecStaff::deleteAll('task_assign_elec_id=' . $this->id);
//        foreach ((array) $userIds as $userId) {
//            $model = new TaskAssignElecStaff();
//            $model->user_id = $userId;
//            $model->task_assign_elec_id = $this->id;
//            if (!$model->save()) {
//                return false;
//            }
//        }
...
//    }
    ...
    private function saveUsers() {
        ...
        try {
            ...
            $existingTaskAssignElecStaff = TaskAssignElecStaff::find()
                    ->where(['task_assign_elec_id' => $this->id])
                    ->all();

            // Normalize types (important for array_diff)
            $existingUserIds = array_map('intval', array_column($existingTaskAssignElecStaff, 'user_id'));
            $userIds = array_map('intval', $userIds);

            // Compare sets
            $newTaskAssignElecStaff = array_diff($userIds, $existingUserIds);          // New to add
            $deleteExistingTaskAssignElecStaff = array_diff($existingUserIds, $userIds); // To delete
            // Add new assignment
            $newAssignment = false;
            foreach ($newTaskAssignElecStaff as $userId) {
                $model = new TaskAssignElecStaff();
                $model->user_id = $userId;
                $model->task_assign_elec_id = $this->id;
                ...
                $this->updateElecStaffCompleteQty($model);
                $newAssignment = true;
            }
            ...
            foreach ($existingTaskAssignElecStaff as $existing) {
                $this->updateElecStaffCompleteQty($existing); // refresh existing qty
            }

            // Handle deletion
            foreach ($existingTaskAssignElecStaff as $existing) {
                if (!in_array((int) $existing->user_id, $deleteExistingTaskAssignElecStaff)) {
                    continue; // keep this user
                }

                // Check if this staff has completion records
                $hasCompletion = TaskAssignElecStaffComplete::find()
                        ->where(['task_assign_elec_staff_id' => $existing->id])
                        ->exists();

                if (!$hasCompletion) {
                    // No completion — safe to delete only within this task
                    TaskAssignElecStaff::deleteAll([
                        'user_id' => $existing->user_id,
                        'task_assign_elec_id' => $this->id, // ensure only current task’s staff are deleted
                    ]);
                }

                $this->updateElecStaffCompleteQty($existing);
            }


            $currentAssignedStaffs = TaskAssignElecStaff::find()->where(['task_assign_elec_id' => $this->id])->all();
            foreach ($currentAssignedStaffs as $current) {
                $this->updateElecStaffCompleteQty($current);
            }

            $completeQty = TaskAssignElecStaff::find()->where(['task_assign_elec_id' => $this->id])->sum('complete_qty_over_total_staff');
    ...
    public function updatePanelCalculation() {
        ...
        if (!$panel->updateElecProgressPercent()) {
            \common\models\myTools\Mydebug::dumpFileA("Error at updateElecProgressPercent");
            return false;
        }

        if (!$project->updateAvgElecProgressPercent()) {
            \common\models\myTools\Mydebug::dumpFileA("Error at updateAvgElecProgressPercent");
            return false;
        }

        if (!$panel->checkPanelElecWorkStatus()) {
            \common\models\myTools\Mydebug::dumpFileA("Error at checkPanelElecWorkStatus");
            return false;
        }
    ...
    public function bulkUpdateThisUserTaskOnHand() {
        $allAssignee = array_column(TaskAssignElecStaff::find()->select(['user_id'])->where(['task_assign_elec_id' => $this->id])->asArray()->all(), 'user_id');
...
//        $completed = TaskAssignElecComplete::find()->where(['task_assign_elec_id' => $this->id])->sum('quantity');
//        $this->complete_qty = $completed;
//
//        return $this->update(false);
//    }
    public function updateCompleteQtyInAssignment() {
        $qtyCompletedByAllStaff = TaskAssignElecStaff::find()->where(['task_assign_elec_id' => $this->id])->sum('complete_qty_over_total_staff');
...
//        $completePanel = new TaskAssignElecComplete();
//        $completePanel->task_assign_elec_id = $taskAssignId;
...
////        }
////
////        $completedTaskTrail = new TaskAssignElecCompleteTrail();
////        $completedTaskTrail->attributes = $completePanel->attributes;
////        $completedTaskTrail->task_assign_elec_complete_id = $completePanel->id;
...
//    }
    ...
    public function saveCompleteTask($addComment, $post) {
        ...
        foreach ($post as $taskAssignElecStaffId => $quantity) {
            $completeByStaff = new TaskAssignElecStaffComplete();
            $completeByStaff->task_assign_elec_staff_id = $taskAssignElecStaffId;
    ...
    //Kindly ensure the migrateUpdateElecStaffQty function is also updated if modifications are made to this function
    public function updateElecStaffQty($taskAssignId, $post) {
        $allSaved = true;

        foreach ($post as $taskAssignElecStaffId => $quantity) {
            $taskAssignElecStaff = TaskAssignElecStaff::findOne(['id' => $taskAssignElecStaffId, 'task_assign_elec_id' => $taskAssignId]);
            if (!$taskAssignElecStaff) {
                return false;
            }

            if (!$this->updateElecStaffCompleteQty($taskAssignElecStaff)) {
                $allSaved = false;
            }
        }
        return $allSaved;
    ...
    public function migrateUpdateElecStaffQty($taskAssignId, $post) {
        $allSaved = true;

        foreach ($post as $taskAssignElecStaffId) {
            $taskAssignElecStaff = TaskAssignElecStaff::findOne(['id' => $taskAssignElecStaffId, 'task_assign_elec_id' => $taskAssignId]);
            if (!$taskAssignElecStaff) {
                return false;
            }

            if (!$this->migrateUpdateElecStaffCompleteQty($taskAssignElecStaff)) {
                $allSaved = false;
            }
        }
        return $allSaved;
...
//    public function addCompletePanelElec($post) {
//        $maxVal = $this->quantity - $this->complete_qty;
//        $addVal = $post['TaskAssignElec']['addComplete'] ?? 0;
//        if ($addVal > $maxVal) {
...
//        $transaction = Yii::$app->db->beginTransaction();
//        try {
//            if (!$this->saveCompleteTask($this->id, $addVal, $post['TaskAssignElec']['extraComment'])) {
//                $transaction->rollBack();
...
//    }
    ...
    public function addCompletePanelElec($post) {
        ...
        $totalAssignedStaff = TaskAssignElecStaff::find()->where(['task_assign_elec_id' => $this->id])->count();
        foreach ($post['staffComplete'] as $taskAssignElecStaffId => $quantity) {
        ...
        $transaction = Yii::$app->db->beginTransaction();
        try {
            if (!$this->saveCompleteTask($post['TaskAssignElec']['extraComment'], $post['staffComplete'])) {
                $transaction->rollBack();
                \common\models\myTools\FlashHandler::err('Failed to save complete task.');
                return 'Failed to save complete task.';
            }

            if (!$this->updateElecStaffQty($this->id, $post['staffComplete'])) {
                $transaction->rollBack();
                \common\models\myTools\FlashHandler::err('Failed to save complete task');
                return 'Failed to save complete task';
            }
    ...
    public function updateCompleteQtyAfterRevert($taskAssignElecStaff, $useTransaction = true) {
        if ($useTransaction) {
            $transaction = Yii::$app->db->beginTransaction();
        }
        try {
            if (!$this->updateElecStaffQtyAfterRevert($taskAssignElecStaff)) {
                if ($useTransaction)
                    $transaction->rollBack();
                return false;
            }
    ...
    public function updateElecStaffQtyAfterRevert($taskAssignElecStaff) {
        if (!$taskAssignElecStaff) {
            return false;
        }
    ...
    private function updateElecStaffCompleteQty($taskAssignElecStaff) {
        if (!$taskAssignElecStaff) {
            return false;
        }

        $totalCompletedTask = TaskAssignElecStaffComplete::find()->where(['task_assign_elec_staff_id' => $taskAssignElecStaff->id])->sum('quantity');
        if ($totalCompletedTask > $this->quantity) {
            return false;
        }

        $taskAssignElecStaff->complete_qty = $totalCompletedTask ?: 0;
        $totalAssignedStaff = TaskAssignElecStaff::find()->where(['task_assign_elec_id' => $taskAssignElecStaff->task_assign_elec_id])->count();
        $taskAssignElecStaff->complete_qty_over_total_staff = round(($taskAssignElecStaff->complete_qty / $totalAssignedStaff), 4);

        if ($totalCompletedTask == $this->quantity) {
            $taskAssignElecStaff->complete_date = MyFormatter::fromDateRead_toDateSQL(date('Y-m-d'));
        }

        if (!$taskAssignElecStaff->save()) {
            \common\models\myTools\Mydebug::dumpFileW($taskAssignElecStaff->getErrors());
            return false;
        }
    ...
    private function migrateUpdateElecStaffCompleteQty($taskAssignElecStaff) {
        if (!$taskAssignElecStaff) {
            return false;
        }
        ...
        if ($totalCompletedTask == $this->quantity && $this->complete_date !== null) {
            $taskAssignElecStaff->complete_date = $this->complete_date;
        }
    ...
    public function updatePanelDefectStaff() {
        $taskAssignedStaffs = $this->taskAssignElecStaff;
        ...
        $transaction = Yii::$app->db->beginTransaction();
        try {
            $productionElecTasksErrors = ProductionElecTasksError::find()->where(['production_elec_task_id' => $this->prod_elec_task_id])->all();

            foreach ($productionElecTasksErrors as $error) {
                /** Remove unassigned staff */
                ProductionElecTasksErrorStaff::deleteAll([
                    'and',
                ...
                foreach ($currentStaffIds as $staffId) {
                    ...
                    $exists = ProductionElecTasksErrorStaff::find()->where(['production_elec_tasks_error_id' => $error->id, 'staff_id' => $staffId])->exists();
                    if (!$exists) {
                ...
                $unreadCount = ProductionElecTasksErrorStaff::find()->where(['production_elec_tasks_error_id' => $error->id, 'is_read' => 1])->count();
            ...
            $transaction->rollBack();
            Yii::error($e->getMessage(), 'ELEC_DEFECT_UPDATE');
            return false;
        }
    }
}
